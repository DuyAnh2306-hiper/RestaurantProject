...
//    final String REGEX_SODT = "0\\d{9}";
    ...
    private void initComponents() {
        ...
        btnDoiBan.setFont(new java.awt.Font("Lucida Grande", 1, 12)); // NOI18N
        btnDoiBan.setForeground(new java.awt.Color(255, 51, 0));
...
//        this.tongHoaDon();
    ...
    private void txtSoDTFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtSoDTFocusLost
        ...
        if (kh != null) {
            if (MsgBox.confirm(this, "Mã khách hàng này đã tồn tại! Bạn có muốn nhập mới không?")) {
                ...
                this.setFormKhachHang(kh.getSoDT());
    ...
    private void btnDoiBanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDoiBanActionPerformed
        // TODO add your handling code here:
        DatBan.soBan = Integer.parseInt(lblSoBan.getText());
        new DatBan().setVisible(true);
//            this.dispose();
    ...
    void setFormKhachHang(String soDT) {
        ...
        if (kh != null) {
            ...
            if (kh.getNgaySinh() == null) {
                ...
                txtNgaySinh.setText(XDate.dateParseString(kh.getNgaySinh(), "dd/MM/yyyy HH:mm:ss aa"));
            }
...
//            System.out.println(hd.getMaKhuyenMai());
//            QLKhuyenMai.fillAllToCbo(cboKhuyenMai);
    ...
    KhachHang getFormKhachHang() {
        ...
        entity.setDiaChi(txtNgaySinh.getText().trim());
        return entity;
    }
    ...
    /**
     * CHƯA XỬ LÝ Khi tìm thông tin hóa đơn, trong chi tiết hóa đơn sẽ có cả
     * thông tin khách hàng và nhân viên handle khi tìm thấy hóa đơn, sẽ kiểm
     * tra xem phần khách hàng có đang rỗng ko, nếu rỗng thì gán thông tin khách
     * hàng trong hóa đơn chi tiết vào hóa đơn, nếu không rỗng, ktra thông tin
     * khách hàng trong hóa đơn đang tìm có trùng khớp không, hỏi cho người dùng
     * confirm, [OK] là gán toàn bộ, [CANCEL] bỏ qua lệnh gán Đối với ngày lặp
     * hóa đơn, tạm thời chỉ xử lý phần ngày, giờ sẽ không đổi, vẫn theo giờ hệ
     * thống
     *
     */
...
//                System.out.println("GiaTriMKHD: " + giaTri);
...
//                                System.out.println("GiaTriMKCT: " + giaTri);
                            }
                        }
                    }
...
//        System.out.println(maHoaDon.substring(4));
        if (hd == null) {
            edit = true;
